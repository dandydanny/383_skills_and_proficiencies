...
# create a few users
s = User.create :name => 'Dev Bootcamp Student', :email => 'me@example.com', :password => 'password'
# s.proficiencies.create(skill_id: rand(1..10), experience: rand(1..5), education: [true, false].sample)

5.times do
  u = User.create :name => Faker::Name.name, :email => Faker::Internet.email, :password => 'password'
  # Associate some skills to users
  2.times do
    u.proficiencies.create(skill_id: rand(1..10), experience: rand(1..10), education: [true, false].sample)
  end
end
